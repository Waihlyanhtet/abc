import telebot, asyncio, aiohttp, json, base64, random, re, os, string, time, uuid, cv2, ddddocr, numpy as np
from telebot.async_telebot import AsyncTeleBot
from aiohttp import web
from datetime import datetime, timedelta, timezone

BOT_TOKEN = '8858687919:AAHynmJMy-61_UtDridL4xiMTaB-_gbuCd0'
ADMIN_ID = "6417845887"  # သင့် ID ထည့်ပါ
...
async def handle_key(message):
    # Admin အားလုံးကို approve လုပ်
    approve[message.chat.id] = True
    user_data[message.chat.id] = {}
    await bot.reply_to(message, "✅ Key approved (admin mode). /input ဖြင့် Session URL ထည့်ပါ။")
...
async def handle_result(message):
    if str(message.chat.id) != ADMIN_ID:
        ...
        return
    results = load_local_json("result.json")
...
async def status(message):
    ...
    await bot.reply_to(message, f"📊 Status\n⏱ {hours}h {minutes}m {seconds}s\n🔍 Active: {active_scans}")
...
async def handle_input(message):
    ...
    if message.chat.id in user_data:
        ...
        if await check_session_url(session_url=url):
            ...
            await bot.reply_to(message, "Session URL သိမ်းပြီး။ /scan 6,7,8,all,ascii-lower သုံးပါ။")
        else:
            await bot.reply_to(message, "Session URL မှားနေပါသည်။")
...
async def scan(message):
    ...
    if chat_id not in user_data or 'session_url' not in user_data[chat_id]:
        await bot.reply_to(message, "/input ကိုအရင်ထည့်ပါ။")
        return
    if chat_id in scan_tasks and not scan_tasks[chat_id]["task"].done():
        await bot.reply_to(message, "Scan လုပ်နေပြီးသား။")
        return
...
async def perform_check(session_url, code, chat_id, scan_id=None, recheck=False, message=None):
        ...
        async with aiohttp.ClientSession(connector=_connector, connector_owner=False, cookie_jar=aiohttp.CookieJar(), timeout=timeout) as task_session:
            ...
            try:
                async with task_session.post(post_url, json=data, headers=headers) as req:
                    ...
                    print(f"code={code} attempt={_attempt+1} status={req.status} resp={resp_json}")
            except Exception as e:
                print(f"error: {e}")
                return
    ...
    if 'logonUrl' in response:
        ...
        save_local_json("result.json", results)
        if message:
            try:
