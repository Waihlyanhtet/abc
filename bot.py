...
# === Key system removed: always approve ===
...
# === Removed: listkeys, delkey, genkey, result, recheck, status commands ===
...
# === Removed: github_update_scheduler (no GitHub) ===
# === SUCCESS_CODE now only stores locally, no file writes ===
async def save_success_code(chat_id, code):
...
async def perform_check(session_url, code, chat_id, scan_id=None, recheck=False, message=None):
    ...
    if 'logonUrl' in response:
        ...
        # Removed GitHub push, only local
        if message:
            try:
...
async def main():
    ...
    try:
        ...
        # Removed github_update_scheduler
        await start_polling()
    finally:
        await session.close()
