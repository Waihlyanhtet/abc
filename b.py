...
def download_file_button(file_content, filename, caption):
    markup = InlineKeyboardMarkup()
    markup.add(InlineKeyboardButton("📥 Download Success Codes", callback_data=f"download_{filename}"))
    return markup
...
def process_file_check(message):
    ...
    # Add download button if success codes exist
    if success_codes:
        markup = InlineKeyboardMarkup()
        markup.add(InlineKeyboardButton("📥 Download Success Codes", callback_data="download_success.txt"))
